...
const App = (() => {
  ...
  const state = {
    ...
    maxBookableDate: null,
    availableSlots: {},
    calendarCache: {}, // 週ごとのキャッシュ
    pendingTel: null,   // SMS認証中の電話番号
    pendingPinId: null, // SMS認証中のpinId
  };
  ...
  const renderCalendar = () => {
    ...
    // 週表示ラベル（maxBookableDate月内の最終日まで）
    const visibleDates = maxMonthEnd ? dates.filter(d => new Date(d) <= maxMonthEnd) : dates;
    const first = dates[0];
